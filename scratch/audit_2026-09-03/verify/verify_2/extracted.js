    ...
    const CACHE_TIMEOUT_MS = 2000, MAX_BODY = 2000000, EXPECT_COUNT = 226, EXPECT2_COUNT = 135;
    ...
    const KIND_CHIP = {
      'Хотел': 'хотел', 'Семеен хотел': 'хотел',
      'хотел · без категоризация': 'хотел', 'апарт-хотел': 'хотел',
      'училище': 'училище', 'университет': 'университет', 'болница': 'болница',
      'ДКЦ': 'ДКЦ', 'хоспис': 'хоспис', 'детска градина': 'детска градина'
    };
    // К1 (§12) - the three colour families over the groups above; data-grp is all
    // the row, the header and the pin carry, the colours live in the CSS. A group
    // missing here falls back to the hotel orange, never to no colour at all.
    const GROUP_COLOUR = {
      'Хотели': 'hotel',
      'Училища': 'edu', 'Университети': 'edu', 'Детски градини': 'edu',
      'Болници': 'health', 'ДКЦ': 'health', 'Хосписи': 'health'
    };
    ...
    function search(q) {
      ...
      if (hasKey) {                              // А3′: key + a purely zone/kind rest = filtered list
