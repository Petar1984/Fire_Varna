#!/usr/bin/env python3
"""Remove NAT-5354 (`coord_28.04320_43.28940`), reported missing in issue #758.
...
Not a duplicate: the nearest record is 20.3 m away, and it is the Golden Sands
hydrant the SAME reporter confirmed exists in #759/#760 - so he treated the two
as separate points on the same walk, and this one simply was not there.
...
NAT-5354 travels with the archive copy only; it is not grafted onto a neighbour.
...
TARGET = "coord_28.04320_43.28940"
ISSUE = 758
EXPECTED_BEFORE = 7413
EXPECTED_AFTER = 7412
EXPECTED_ARCHIVE_BEFORE = 37
...
EXPECTED_DELTA = {
    ...
    "reported": (2, 1), "notes": (81, 80), "typed": (2741, 2740),
}
REASON = (
    "Докладван като липсващ от 01 РСПБЗН (#758): „Няма хидрант или е затрупан.“ "
    "Не е дубликат — най-близкият съсед е на 20.3 м и е хидрантът от Златни "
    "пясъци, който СЪЩИЯТ докладчик потвърди в #759/#760 като съществуващ, тоест "
    "той е третирал двете точки като различни на един и същи обход. Стандартното "
    "поведение би бил само жълт флаг; Петър реши да се довери на институционалния "
    "докладчик, както при #491, #580 и #624. NAT-5354 остава само в този архив и "
    "НЕ се прехвърля към съсед. Забележка: бележката допуска и „затрупан“, тоест "
    "хидрантът може да е под насип — премахването е решение на Петър при това "
    "знание."
)
...
def main(apply_writes: bool) -> int:
    ...
    assert victim.get("legacy_ids") == ["NAT-5354"], victim
    ...
    print("REMOVING", TARGET, "(NAT-5354, надземен, yellow)")
