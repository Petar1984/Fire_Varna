...
**Date:** 2026-09-03 · **last verified against commit** c5ef1b2 (`lot1-client`, the ЛОТ 1 client branch in a separate worktree, rebased onto `main`; `main` is ahead of `origin/main` by the places-search cycle's commits and the branch is 4 ahead of `main` — see the table — until Petar's push) · verified by: Opus (executor), audited by: Opus (auditor, C6, C14, C14b, C14c and the ЛОТ 1 client audit) · signed: __
...
- Gate 2 on the ЛОТ 1 client half (branch `lot1-client`, top c5ef1b2): the list to sign does not come from here — it comes from the reference preview of the running simulation (`lot1_reference_preview_v2.md`, not written yet) and from the varna_3d board (`docs/audits/БОРД_места_03.09.md` there, @ d9c6c06). Until both are on the table the branch stays unmerged.
...
| commits on `main` | 248 | `git rev-list --count main` |
...
| commits ahead of `origin/main` (the places-search cycle, unpushed) | 34 | `git rev-list --count origin/main..main` |
| commits on `lot1-client` not on `main` (the ЛОТ 1 client half: F1-к, F2-к0, F2-к, F3-к; this docs commit makes it 5) | 4 | `git rev-list --count main..lot1-client` |
...
| `index.html` bytes (on `lot1-client`; on `main` 526153 — `git show main:index.html \| wc -c`) | 530484 | `wc -c < index.html` |
...
| tests (on `lot1-client`; 176 on `main`) | Ran 184 tests · OK | `PYTHONIOENCODING=utf-8 python -m unittest discover -s tests 2>&1 \| tail -3` |
