...
| **ИА-О31** | **Втора сверка на Архитекта (08.09 нощ, след Astra S40 и Кими К39; пространството на отклоненията се затваря на ИА-О31).** (1) Хешът на суровия отговор на речника е махнат от §1 (частен провенанс не стои в проследен план); (2) §3.Г.1 записите в `_meta.excluded` = петорката на v1 + `delivered`/`status` (+ `district_witness_note` за пристанището), без `reason`; (3) §3.Г.8 и Д2: `source_terms.imot_bg` = ТРИ ключа навсякъде; (4) §3 ред `license/license_terms/license_label/attribution` носи дословните стойности; (5) ИА-19: полярността е една навсякъде — изход 0 = намерена координата = ИА-STOP 17, изход 2 = ИА-STOP 5, изход 1 = чисто; (6) стъпка 0 не пуска Н7 (тестът регенерира фикстури в репото) и не изисква стария хънк на Д3; стъпка 3 съди `!`-правилото и `ls-files --error-unmatch`, не изхода на `check-ignore`; (7) К6 (Петър) предхожда К5 (четците към архива); (8) стъпка 12 доказва, че доставеният файл е байт-равен на прогон a (`cmp`); (9) Р4 е обявено отклонение от ADR 010 D4 (`no_boundary` без Feature) с амандамент в Д2; ИА-14 праща четирите жилищни кода без Feature в `rows_without_feature.deferred[]`; (10) „1 025“ са СГРАДИ по тялото, адресните точки ги мери ИА-11; (11) ИБ-19 (присъда за условията преди публикуване) и ИБ-20 (скан за сурови хешове в прозата) добавени в §11. | Астра S40 и Кими К39 са в `scratch/places_search/adresi_astra_S40_08.09.md` и `adresi_kimi_K39_08.09.md`. |
...
1. **Домът `_meta.excluded`, **10 РЕДА** (и **11 ЗАПИСА**):** десетте реда с **`kind_registry = null`** (пет обекта, пет села — поименно в §1.6), всеки с **петорката на v1** — `code`, `display` (= `imot_name` от тялото), `kind` (суровият imot-вид), `registry_action` (= `"not_in_registry"` за петте обекта, `"deferred_village"` за петте села), `why` (= дословният `kind_note` от тялото) — плюс **`delivered: false`**, а за `pristanishte_varna` и `district_witness_note` (**ИА-О31**, К39-2/К39-6: никакъв ключ `reason`, никакви `raion_num`/`imot_name` в записа — те са в Ф7); плюс **един ЕДИНАДЕСЕТИ запис `vinitsa_sever` със `status: "reinstated"`, `delivered: true`** (ИА-О15, Р17), който **не е дом на ред** — редът му е Feature — а е запис на обърнато подписано решение. **Биекцията, която ИА-17 съди: `kind_registry = null` ⇔ запис в `excluded` с `delivered: false`. Записите с `delivered: true` са точно тези, за които §2 носи отклонение — днес един.** За `pristanishte_varna` записът носи и `district_witness_note` (разминаването наш слой ↔ АГКК, §1.7).
...
| `url` | **чистият адрес на списъка на imot.bg за град Варна: `https://www.imot.bg/obiavi/prodazhbi/grad-varna`** — това е `_meta.list_endpoint` на снапшота БЕЗ описателната опашка „(95 with adverts, counts)“, която е бележка, не URL (ИА-О30; Ф1 отрязва всичко от първия интервал нататък) — **непразен и еднакъв по всичките 84** (ИА-О28: смисълът на `url` се сменя от „обектът-извор“ (v1, 60 различни) към „списъкът-извор“ (v2, един)) |
...
|---|---|---|---|
| 0 | Изп | Пълната мярка на §1 **без нито една редакция**. Записва OID-ите на 14-те стейджнати пътя (`git ls-files -s -- <14-те>`), изходите **Н1–Н6, Н8, Н9**, **16-те идентификатора на тестовете** (по списък, без прогон) и **състоянието на Д3** по `git status --porcelain -- docs/sessions/СЪСТОЯНИЕ_Границите_06.09.md` (чист или мръсен — записва се фактът; **ИА-О31:** старият хънк на §20 е комитнат в `0304a3c`, не се изисква). **Н7 (`python -m unittest discover -s tests`) НЕ се пуска на стъпка 0** — `tests/test_quarters_signed_fixtures.py:156` изтрива и регенерира фикстури в репото (Ч); мери се на стъпка 13 в еднократното `git worktree`. | разлика в Н1–Н6, Н8, Н9 или в 16-те идентификатора = **ИА-STOP 2** |
| 1 | **Петър** | **Gate 1** — подписва този план и **двайсет и едното** решения на §9. | без подпис нищо не тръгва |
| 2 | Изп | **Записва подписания текст като `Д1`** (върху редакция 2, ИА-О22) → пуска **ИА-19** върху Д1 → `git add -- docs/plans/ПЛАН_Картата_imot_И-А_08.09.md` → комит **К1** → `b3_g11.py HEAD --cached` = 0 | ИА-19 с изход **0** (`grep` намери координата) = **ИА-STOP 17**; изход **2** (грешка на командата) = **ИА-STOP 5**; изход 1 = чисто; b3_g11 ≠ 0 = **ИА-STOP 9** |
| 3 | Изп | Копира Ф5, Ф6 в `varna_3d/data/`; прави **единствената** редакция Т1; `git check-ignore -v` за Ф5, Ф6 → **печата разрешаващото `!`-правило на Т1** (ИА-О31: при `!`-правило командата НЕ мълчи; изходният код не е критерият) → `git add -- data/imot_bg_varna_polygons_08.09.geojson data/imot_bg_varna_dictionary_08.09.json .gitignore` → **`git ls-files --error-unmatch -- <двата>` = 0** (това е критерият). **Не комитва.** | `check-ignore` без `!`-правило или `ls-files --error-unmatch` ≠ 0 = **ИА-STOP 11** |
...
| 17 | Изп | Комити **К8** и **К9** във Fire_Varna → **ИА-19 върху Д2 и Д3 преди всеки** → `b3_g11.py HEAD --cached` = 0 след всеки. | ИА-19 с изход **0** (`grep` намери координата) = **ИА-STOP 17**; изход **2** (грешка на командата) = **ИА-STOP 5**; изход 1 = чисто; b3_g11 ≠ 0 = **ИА-STOP 9** |
...
| **Р3** | **Петте „обекта“** | Тялото им дава **`kind_registry = null`** с довод „обект: не е клетка → без вид“ (Ч). И петте имат полигон и сгради; и петте **липсват от регистъра**. **Държат 1 025 СГРАДИ** по колоната `buildings` на тялото (355/354/274/26/16, М — това са сгради, не адресни точки; **адресните точки в тези клетки ги измерва ИА-11 на стъпка 9 и числото влиза в отчета** — ИА-О31, К39-3). С тях покритието по сгради е 97,18 %, без тях — 95,90 %. | **ИЗКЛЮЧЕНИ.** Стоят поименно в **`_meta.excluded[]`** с `delivered: false` и довода си дословно; точките им са клас `excluded_cell`. **Цената е назована: 1 025 сгради (1,27 %) остават без квартална дума; адресните точки в тях — по ИА-11.** |
| **Р4** | **Трите без полигон** — 5848, 5866, 6294 | Речникът ги знае, imot няма полигон; `buildings = null`, `area_km2 = null` (М). `malka_chaika` е в регистъра като `м-т`, а тялото му дава `kind_registry = "кв"` (М) — разминаване, което днес не удря гейт, защото редът е `no_boundary`. | **`no_boundary` — БЕЗ Feature.** Изброяват се в `_meta.rows_without_feature.no_boundary[]` с довод. **Не** се прави Feature с `geometry: null` — **обявено отклонение от ADR 010 D4** (`:44` предвижда Feature с `geometry: null` за `no_boundary`): в v2 гейтовете броят Feature-и = клетки, затова `no_boundary` живее в `_meta.rows_without_feature.no_boundary[]` (ИА-О31; амандаментът на D4 влиза в Д2 с К8). Разминаването по `malka_chaika` влиза в Ф10. |
...
**ИБ-20 · Скан за суров провенанс в прозата.** ИА-19 хваща координати; никой гейт не хваща 64-знакови хешове на сурови отговори в проследените `.md`. И-А ги маха на ръка (ИА-О31, S40 бл. 1); И-Б добавя към ИА-19 израз за `[0-9a-f]{64}`, който допуска само хешовете на БЛОБОВЕ (`git show`) и на проследени файлове.
